 ...
 * $Header: $
 * $Revision: 462499 $
 * $Date: 2006-10-10 20:57:36 +0200 (mar., 10 oct. 2006) $
 * 
...
namespace IBatisNet.DataAccess.Configuration
{
    ...
    public class DomDaoManagerBuilder
    {
        ...
        /// </summary>
        private static readonly string XML_CONFIG_PROVIDERS = "daoConfig/providers";
...
//		[MethodImpl(MethodImplOptions.Synchronized)]
        public void BuildDaoManagers(XmlDocument document, bool useConfigFileWatcher)
        {
            ...
            configurationScope.XmlNamespaceManager =
                new XmlNamespaceManager(configurationScope.DaoConfigDocument.NameTable);
        ...
        private void GetProviders(ConfigurationScope configurationScope)
        {
            ...
            providersNode =
                configurationScope.DaoConfigDocument.SelectSingleNode(ApplyNamespacePrefix(XML_CONFIG_PROVIDERS),
                    configurationScope.XmlNamespaceManager);
            ...
            foreach (XmlNode node in xmlProviders.SelectNodes(ApplyProviderNamespacePrefix(XML_PROVIDER),
                configurationScope.XmlNamespaceManager))
            {
                configurationScope.ErrorContext.Resource = node.InnerXml;
        ...
        private void GetDaoSessionHandlers(ConfigurationScope configurationScope)
        {
            ...
            daoSessionHandlersNode = configurationScope.DaoConfigDocument.SelectSingleNode(
                ApplyNamespacePrefix(XML_DAO_SESSION_HANDLERS), configurationScope.XmlNamespaceManager);

            if (daoSessionHandlersNode != null)
                foreach (XmlNode node in daoSessionHandlersNode.SelectNodes(ApplyNamespacePrefix(XML_HANDLER),
                    configurationScope.XmlNamespaceManager))
                {
                    configurationScope.ErrorContext.Resource = node.InnerXml;

                    DaoSessionHandler daoSessionHandler =
                        DaoSessionHandlerDeSerializer.Deserialize(node, configurationScope);
                    ...
                    if (daoSessionHandler.IsDefault)
        ...
        private void GetContexts(ConfigurationScope configurationScope)
        {
            ...
            foreach (XmlNode contextNode in configurationScope.DaoConfigDocument.SelectNodes(
                ApplyNamespacePrefix(XML_DAO_CONTEXT), configurationScope.XmlNamespaceManager))
            {
                ...
                XmlNode nodeSessionHandler = contextNode.SelectSingleNode(ApplyNamespacePrefix(XML_DAO_SESSION_HANDLER),
                    configurationScope.XmlNamespaceManager);
                ...
                {
                    configurationScope.ErrorContext.Resource = nodeSessionHandler.InnerXml;

                    typeSessionHandler =
                        configurationScope.DaoSectionHandlers[
                            nodeSessionHandler.Attributes[ID_ATTRIBUTE].Value] as Type;

                    // Parse property node
                    foreach (XmlNode nodeProperty in nodeSessionHandler.SelectNodes(ApplyNamespacePrefix(XML_PROPERTY),
                        configurationScope.XmlNamespaceManager))
                        resources.Add(nodeProperty.Attributes["name"].Value,
                            NodeUtils.ParsePropertyTokens(nodeProperty.Attributes["value"].Value,
                ...
                catch (Exception e)
                {
                    throw new ConfigurationException(
                        string.Format(
                            "DaoManager could not configure DaoSessionHandler. DaoSessionHandler of type \"{0}\", failed. Cause: {1}",
                            typeSessionHandler.Name, e.Message), e
                    );
                }
        ...
        private void ParseGlobalProperties(ConfigurationScope configurationScope)
        {
            XmlNode nodeProperties =
                configurationScope.NodeContext.SelectSingleNode(ApplyNamespacePrefix(XML_PROPERTIES),
                    configurationScope.XmlNamespaceManager);
            ...
            if (nodeProperties != null)
            {
                ...
                {
                    foreach (XmlNode propertyNode in nodeProperties.SelectNodes(ApplyNamespacePrefix(XML_PROPERTY),
                        configurationScope.XmlNamespaceManager))
                    {
                        ...
                        if (keyAttrib != null && valueAttrib != null)
                        {
                            ...
                            _logger.Info(string.Format("Add property \"{0}\" value \"{1}\"", keyAttrib.Value,
                                valueAttrib.Value));
                        }
                        else
                        {
                            ...
                            XmlDocument propertiesConfig =
                                Resources.GetAsXmlDocument(propertyNode, configurationScope.Properties);
                            ...
                            foreach (XmlNode node in propertiesConfig.SelectNodes(XML_SETTINGS_ADD))
                            {
                                ...
                                _logger.Info(string.Format("Add property \"{0}\" value \"{1}\"",
                                    node.Attributes[KEY_ATTRIBUTE].Value, node.Attributes[VALUE_ATTRIBUTE].Value));
                            }
                        }
                    }
                }
                else
                {
                    ...
                    configurationScope.ErrorContext.Resource = nodeProperties.OuterXml;

                    // Load the file defined by the attribute
                    XmlDocument propertiesConfig =
                        Resources.GetAsXmlDocument(nodeProperties, configurationScope.Properties);
...
//				}
        ...
        {
            XmlNode node = configurationScope.NodeContext.SelectSingleNode(ApplyNamespacePrefix(XML_DATABASE_PROVIDER),
                configurationScope.XmlNamespaceManager);
            ...
            if (node != null)
            {
                configurationScope.ErrorContext.Resource = node.OuterXml;
                string providerName =
                    NodeUtils.ParsePropertyTokens(node.Attributes["name"].Value, configurationScope.Properties);
            ...
            if (configurationScope.Providers.Contains(DEFAULT_PROVIDER_NAME))
...
//		}
        ...
        private DataSource ParseDataSource(ConfigurationScope configurationScope)
        {
            ...
            XmlNode node = configurationScope.NodeContext.SelectSingleNode(
                ApplyNamespacePrefix(XML_DATABASE_DATASOURCE), configurationScope.XmlNamespaceManager);
...
//				(DataSource)serializer.Deserialize(new XmlNodeReader(node));
            ...
            dataSource.ConnectionString =
                NodeUtils.ParsePropertyTokens(dataSource.ConnectionString, configurationScope.Properties);
        ...
        private void ParseDaoFactory(ConfigurationScope configurationScope, DaoManager daoManager)
        {
            ...
            foreach (XmlNode node in configurationScope.NodeContext.SelectNodes(ApplyNamespacePrefix(XML_DAO),
                configurationScope.XmlNamespaceManager))
            {
        ...
        {
            return DAO_NAMESPACE_PREFIX + ":" + elementName.Replace("/", "/" + DAO_NAMESPACE_PREFIX + ":");
        }

...
/*****************************************************************************
 * $Revision: 416031 $
 * $LastChangedDate: 2006-06-21 18:40:27 +0200 (mer., 21 juin 2006) $
...
namespace IBatisNet.DataMapper.Configuration.ParameterMapping
{
    ...
    public class ParameterProperty
    {
        ...
        [NonSerialized] private string _nullValue; //string.Empty;//null;

        [NonSerialized] private string _propertyName = string.Empty;

        [NonSerialized] private ParameterDirection _direction = ParameterDirection.Input;

        [NonSerialized] private string _directionAttribute = string.Empty;

        [NonSerialized] private string _dbType;

        [NonSerialized] private int _size = -1;

        [NonSerialized] private byte _scale;

        [NonSerialized] private byte _precision;

        [NonSerialized] private string _columnName = string.Empty; // used only for store procedure
        ...
        [NonSerialized] private IGetAccessor _getAccessor;
        ...
        public void Initialize(IScope scope, Type parameterClass)
        {
                ...
                && !scope.DataExchangeFactory.TypeHandlerFactory.IsSimpleType(parameterClass)) // value property
            {
                if (!_isComplexMemberName)
                {
                    IGetAccessorFactory getAccessorFactory =
                        scope.DataExchangeFactory.AccessorFactory.GetAccessorFactory;
            ...
            scope.ErrorContext.MoreInfo = "Check the parameter mapping typeHandler attribute '" + CallBackName +
            ...
            else
            {
                if (CLRType.Length == 0) // Unknown
                {
                    if (scope.DataExchangeFactory.TypeHandlerFactory.IsSimpleType(parameterClass))
                        _typeHandler =
                            scope.DataExchangeFactory.TypeHandlerFactory.GetTypeHandler(parameterClass, _dbType);
                    else
        ...
        /// <summary>
        ///     Determines whether the specified <see cref="System.Object"></see> is equal to the current
        ///     <see cref="System.Object"></see>.
        /// </summary>
        /// <param name="obj">The <see cref="System.Object"></see> to compare with the current <see cref="System.Object"></see>.</param>
        /// <returns>
        ///     true if the specified <see cref="System.Object"></see> is equal to the current <see cref="System.Object"></see>;
        ///     otherwise, false.
        /// </returns>
        ...
        /// <summary>
        ///     Serves as a hash function for a particular type. <see cref="System.Object.GetHashCode"></see> is suitable for use

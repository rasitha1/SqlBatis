...
namespace IBatisNet.DataMapper.Configuration.Sql.SimpleDynamic
{
    ...
    internal sealed class SimpleDynamicSql : ISql
    {
        ...
        private readonly string _simpleSqlStatement = string.Empty;
            ...
            while (enumerator.MoveNext())
            {
                ...
                if (ELEMENT_TOKEN.Equals(lastToken))
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (_dataExchangeFactory.TypeHandlerFactory.IsSimpleType(parameterObject.GetType()))
                                value = parameterObject;
                            else
                                value = ObjectProbe.GetMemberValue(parameterObject, token,
                                    _dataExchangeFactory.AccessorFactory);
                        }

                        if (value != null) newSql.Append(value);

                        enumerator.MoveNext();
                        token = ((string) enumerator.Current);

                        if (!ELEMENT_TOKEN.Equals(token))
                            throw new DataMapperException("Unterminated dynamic element in sql (" +
                                                          _simpleSqlStatement + ").");
                        token = null;
                    }
                }
                ...
                {
                    if (!ELEMENT_TOKEN.Equals(token)) newSql.Append(token);
                }

 ...
 * $Header: $
 * $Revision: 502295 $
 * $Date: 2007-02-01 18:28:47 +0100 (jeu., 01 févr. 2007) $
 * 
...
namespace IBatisNet.DataMapper.Configuration.Statements
{
    ...
    public class PreparedStatementFactory
    {
        ...
        public PreparedStatementFactory(ISqlMapSession session, RequestScope request, IStatement statement,
            string commandText)
        {
            _session = session;
            _request = request;
            ...
            else if (_statement.CommandType == CommandType.StoredProcedure) // StoredProcedure
            {
                if (_request.ParameterMap == null) // No parameterMap --> error
                    throw new DataMapperException(
                        "A procedure statement tag must have a parameterMap attribute, which is not the case for the procedure '" +
                        _statement.Id + ".");
                ...
                if (_session.DataSource.DbProvider.IsObdc)
                {
                    ...
                    if (_preparedStatement.DbParameters.Length > 0)
                    {
                        ...
                        for (int i = 0; i < supIndex; i++) commandTextBuilder.Append("?,");
                        commandTextBuilder.Append("?) }");
                    }
        ...
        private void DiscoverParameter(ISqlMapSession session)
        {
            ...
            for (int i = 0; i < _statement.ParameterMap.Properties.Count; i++)
                sortedDbParameters[i] = Search(session, _preparedStatement.DbParameters,
                    _statement.ParameterMap.Properties[i], i);
            _preparedStatement.DbParameters = sortedDbParameters;
        }

        private IDbDataParameter Search(ISqlMapSession session, IDbDataParameter[] parameters,
            ParameterProperty property, int index)
        {
            if (property.ColumnName.Length > 0)
            {
                for (int i = 0; i < parameters.Length; i++)
                {
                    ...
                    if (property.ColumnName.Equals(parameterName)) return parameters[i];
                }

                throw new IndexOutOfRangeException("The parameter '" + property.ColumnName +
                                                   "' does not exist in the stored procedure '" + _statement.Id +
        ...
        private void CreateParametersForTextCommand()
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (_session.DataSource.DbProvider.SetDbParameterScale) dataParameter.Scale = property.Scale;

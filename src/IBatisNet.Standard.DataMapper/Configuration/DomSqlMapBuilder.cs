 ...
 * $Header: $
 * $Revision: 510185 $
 * $Date: 2007-02-21 21:23:49 +0100 (mer., 21 févr. 2007) $
 * 
...
namespace IBatisNet.DataMapper.Configuration
{
    ...
    public class DomSqlMapBuilder
    {
        ...
        ///     Lazy factory type
        /// </summary>
        private const string ATR_LAZY_FACTORY_TYPE = "lazyFactoryType";
            ...
            bool useConfigFileWatcher, bool isCallFromDao)
        {
            ...
            try
            {
                if (_validateSqlMapConfig) ValidateSchema(document.ChildNodes[1], "SqlMapConfig.xsd");
                Initialize();
        ...
        private void ValidateSchema(XmlNode section, string schemaFileName)
        {
            ...
            try
            {
                ...
                if (xsdFile == null)
                    throw new ConfigurationException("Unable to locate embedded resource [IBatisNet.DataMapper." +
                                                     schemaFileName +
                                                     "]. If you are building from source, verfiy the file is marked as an embedded resource.");
                ...
                if (!_configScope.IsXmlValid)
                    throw new ConfigurationException("Invalid SqlMap.config document. cause :" +
        ...
        public ISqlMapper Build(XmlDocument document, DataSource dataSource, bool useConfigFileWatcher,
        ...
        private void Initialize()
        {
            ...
            {
                _configScope.NodeContext = _configScope.SqlMapConfigDocument.SelectSingleNode(
                    ApplyDataMapperNamespacePrefix(XML_DATAMAPPER_CONFIG_ROOT), _configScope.XmlNamespaceManager);
            ...
            XmlNodeList settings = _configScope.SqlMapConfigDocument.SelectNodes(
                ApplyDataMapperNamespacePrefix(XML_CONFIG_SETTINGS), _configScope.XmlNamespaceManager);
            ...
            if (settings != null)
                foreach (XmlNode setting in settings)
                {
                    ...
                    {
                        string value = NodeUtils.ParsePropertyTokens(
                            setting.Attributes[ATR_USE_STATEMENT_NAMESPACES].Value, _configScope.Properties);
                        _configScope.UseStatementNamespaces = Convert.ToBoolean(value);
                    }

                    if (setting.Attributes[ATR_CACHE_MODELS_ENABLED] != null)
                    {
                        string value = NodeUtils.ParsePropertyTokens(setting.Attributes[ATR_CACHE_MODELS_ENABLED].Value,
                    ...
                    {
                        string value = NodeUtils.ParsePropertyTokens(
                            setting.Attributes[ATR_USE_REFLECTION_OPTIMIZER].Value, _configScope.Properties);
                        _configScope.UseReflectionOptimizer = Convert.ToBoolean(value);
                    }

                    if (setting.Attributes[ATR_VALIDATE_SQLMAP] != null)
                    {
                        string value = NodeUtils.ParsePropertyTokens(setting.Attributes[ATR_VALIDATE_SQLMAP].Value,
                            _configScope.Properties);
                        _configScope.ValidateSqlMap = Convert.ToBoolean(value);
                    }

                    if (setting.Attributes[ATR_LAZY_FACTORY_TYPE] != null)
                    {
                        string value = NodeUtils.ParsePropertyTokens(setting.Attributes[ATR_LAZY_FACTORY_TYPE].Value,
                            _configScope.Properties);
                        _configScope.LazyFactoryType = TypeUtils.ResolveType(value);
                    }
                }

            #endregion

            if (_objectFactory == null) _objectFactory = new ObjectFactory(_configScope.UseReflectionOptimizer);
            ...
            if (_configScope.IsCallFromDao == false) GetProviders();
            ...
            XmlNode nodeDataSource = _configScope.SqlMapConfigDocument.SelectSingleNode(
                ApplyDataMapperNamespacePrefix(XML_DATABASE_DATASOURCE), _configScope.XmlNamespaceManager);
            ...
            else
            {
                if (_configScope.IsCallFromDao == false)
                {
                    ...
                    dataSource.ConnectionString =
                        NodeUtils.ParsePropertyTokens(dataSource.ConnectionString, _configScope.Properties);
            ...
            foreach (XmlNode xmlNode in _configScope.SqlMapConfigDocument.SelectNodes(
                ApplyDataMapperNamespacePrefix(XML_GLOBAL_TYPEALIAS), _configScope.XmlNamespaceManager))
            {
            ...
            foreach (XmlNode xmlNode in _configScope.SqlMapConfigDocument.SelectNodes(
                ApplyDataMapperNamespacePrefix(XML_GLOBAL_TYPEHANDLER), _configScope.XmlNamespaceManager))
                try
                {
                ...
                catch (Exception e)
                {
                    ...
                    throw new ConfigurationException(
                        string.Format(
                            "Error registering TypeHandler class \"{0}\" for handling .Net type \"{1}\" and dbType \"{2}\". Cause: {3}",
            ...
            foreach (XmlNode xmlNode in _configScope.SqlMapConfigDocument.SelectNodes(
                ApplyDataMapperNamespacePrefix(XML_SQLMAP), _configScope.XmlNamespaceManager))
            {
                ...
                if (statementsToRegister != null && statementsToRegister.Count > 0)
                    foreach (string statementName in statementsToRegister)
                    {
                        IMappedStatement mappedStatement =
                            _configScope.SqlMapper.MappedStatements[statementName] as IMappedStatement;
                        ...
                        if (mappedStatement != null)
                        {
                            ...
                            if (_logger.IsDebugEnabled)
                                _logger.Debug("Registering trigger statement [" + mappedStatement.Id +
                                              "] to cache model [" + cacheModel.Id + "]");
                        ...
                        else
                        {
                            if (_logger.IsWarnEnabled)
                                _logger.Warn("Unable to register trigger statement [" + statementName +
                                             "] to cache model [" + cacheModelId + "]. Statement does not exist.");
                        }
                    }
            }
            ...
            foreach (DictionaryEntry entry in _configScope.SqlMapper.ResultMaps)
            {
                ...
                if (resultMap.Discriminator != null) resultMap.Discriminator.Initialize(_configScope);
            }
        ...
        private void GetProviders()
        {
            ...
            providersNode = _configScope.SqlMapConfigDocument.SelectSingleNode(
                ApplyDataMapperNamespacePrefix(XML_CONFIG_PROVIDERS), _configScope.XmlNamespaceManager);
            ...
            foreach (XmlNode node in xmlProviders.SelectNodes(ApplyProviderNamespacePrefix(XML_PROVIDER),
                _configScope.XmlNamespaceManager))
            {
                _configScope.ErrorContext.Resource = node.InnerXml;
                ...
                if (provider.IsEnabled)
                {
                    ...
                    if (provider.IsDefault)
                    {
                        if (_configScope.Providers[DEFAULT_PROVIDER_NAME] == null)
                            ...
                            throw new ConfigurationException(
                                string.Format(
                                    "Error while configuring the Provider named \"{0}\" There can be only one default Provider.",
                                    provider.Name));
                    }
                }
            }
        ...
        private IDbProvider ParseProvider()
        {
            ...
            XmlNode node = _configScope.SqlMapConfigDocument.SelectSingleNode(
                ApplyDataMapperNamespacePrefix(XML_DATABASE_PROVIDER), _configScope.XmlNamespaceManager);

            if (node != null)
            {
                _configScope.ErrorContext.Resource = node.OuterXml;
                string providerName =
                    NodeUtils.ParsePropertyTokens(node.Attributes["name"].Value, _configScope.Properties);
                ...
                throw new ConfigurationException(
                    string.Format(
                        "Error while configuring the Provider named \"{0}\". Cause : The provider is not in 'providers.config' or is not enabled.",
                        providerName));
            }
            ...
            throw new ConfigurationException(
                "Error while configuring the SqlMap. There is no provider marked default in \'providers.config\' file.");
        }
        ...
        private void ConfigureSqlMap()
        {
            ...
            _configScope.ErrorContext.Resource = sqlMapNode.OuterXml;
            ...
            if (_configScope.UseConfigFileWatcher)
                if (sqlMapNode.Attributes["resource"] != null || sqlMapNode.Attributes["url"] != null)
                    ConfigWatcherHandler.AddFileToWatch(
                        Resources.GetFileInfo(
                            Resources.GetValueOfNodeResourceUrl(sqlMapNode, _configScope.Properties)));

            // Load the file 
            _configScope.SqlMapDocument = Resources.GetAsXmlDocument(sqlMapNode, _configScope.Properties);

            if (_configScope.ValidateSqlMap) ValidateSchema(_configScope.SqlMapDocument.ChildNodes[1], "SqlMap.xsd");

            _configScope.SqlMapNamespace = _configScope.SqlMapDocument
                .SelectSingleNode(ApplyMappingNamespacePrefix(XML_MAPPING_ROOT), _configScope.XmlNamespaceManager)
                .Attributes["namespace"].Value;

            #region Load TypeAlias

            foreach (XmlNode xmlNode in _configScope.SqlMapDocument.SelectNodes(
                ApplyMappingNamespacePrefix(XML_TYPEALIAS), _configScope.XmlNamespaceManager))
                TypeAliasDeSerializer.Deserialize(xmlNode, _configScope);
            ...
            foreach (XmlNode xmlNode in _configScope.SqlMapDocument.SelectNodes(
                ApplyMappingNamespacePrefix(XML_RESULTMAP), _configScope.XmlNamespaceManager))
            {
            ...
            foreach (XmlNode xmlNode in _configScope.SqlMapDocument.SelectNodes(
                ApplyMappingNamespacePrefix(XML_PARAMETERMAP), _configScope.XmlNamespaceManager))
            {
            ...
            foreach (XmlNode xmlNode in _configScope.SqlMapDocument.SelectNodes(
                ApplyMappingNamespacePrefix(SQL_STATEMENT), _configScope.XmlNamespaceManager))
            {
                ...
                ApplyMappingNamespacePrefix(XML_STATEMENT), _configScope.XmlNamespaceManager))
            {
                ...
                if (_configScope.UseStatementNamespaces) statement.Id = _configScope.ApplyNamespace(statement.Id);
                ...
                if (statement.CacheModelName != null && statement.CacheModelName.Length > 0 &&
                    _configScope.IsCacheModelsEnabled) mapStatement = new CachingStatement(mappedStatement);
            ...
            Select select;
            foreach (XmlNode xmlNode in _configScope.SqlMapDocument.SelectNodes(ApplyMappingNamespacePrefix(XML_SELECT),
                _configScope.XmlNamespaceManager))
            {
                ...
                if (_configScope.UseStatementNamespaces) select.Id = _configScope.ApplyNamespace(select.Id);
                ...
                IMappedStatement mapStatement = mappedStatement;
                if (select.CacheModelName != null && select.CacheModelName.Length > 0 &&
                    _configScope.IsCacheModelsEnabled) mapStatement = new CachingStatement(mappedStatement);
            ...
            Insert insert;
            foreach (XmlNode xmlNode in _configScope.SqlMapDocument.SelectNodes(ApplyMappingNamespacePrefix(XML_INSERT),
                _configScope.XmlNamespaceManager))
            {
                ...
                if (_configScope.UseStatementNamespaces) insert.Id = _configScope.ApplyNamespace(insert.Id);
                ...
                if (insert.SelectKey != null)
                {
                    ...
                    _configScope.NodeContext = xmlNode.SelectSingleNode(ApplyMappingNamespacePrefix(XML_SELECTKEY),
                        _configScope.XmlNamespaceManager);
            ...
            Update update;
            foreach (XmlNode xmlNode in _configScope.SqlMapDocument.SelectNodes(ApplyMappingNamespacePrefix(XML_UPDATE),
                _configScope.XmlNamespaceManager))
            {
                ...
                if (_configScope.UseStatementNamespaces) update.Id = _configScope.ApplyNamespace(update.Id);
            ...
            Delete delete;
            foreach (XmlNode xmlNode in _configScope.SqlMapDocument.SelectNodes(ApplyMappingNamespacePrefix(XML_DELETE),
                _configScope.XmlNamespaceManager))
            {
                ...
                if (_configScope.UseStatementNamespaces) delete.Id = _configScope.ApplyNamespace(delete.Id);
                ...
                ApplyMappingNamespacePrefix(XML_PROCEDURE), _configScope.XmlNamespaceManager))
            {
                ...
                if (_configScope.UseStatementNamespaces) procedure.Id = _configScope.ApplyNamespace(procedure.Id);
                ...
                IMappedStatement mapStatement = mappedStatement;
                if (procedure.CacheModelName != null && procedure.CacheModelName.Length > 0 &&
                    _configScope.IsCacheModelsEnabled) mapStatement = new CachingStatement(mappedStatement);
            ...
            if (_configScope.IsCacheModelsEnabled)
            {
                    ...
                    ApplyMappingNamespacePrefix(XML_CACHE_MODEL), _configScope.XmlNamespaceManager))
                {
                    ...
                    foreach (XmlNode flushOn in xmlNode.SelectNodes(ApplyMappingNamespacePrefix(XML_FLUSH_ON_EXECUTE),
                        _configScope.XmlNamespaceManager))
                    {
                        ...
                        if (statementNames == null) statementNames = new ArrayList();
                        statementNames.Add(statementName);
                        _configScope.CacheModelFlushOnExecuteStatements[cacheModel.Id] = statementNames;
                    }

                    // Get Properties
                    foreach (XmlNode propertie in xmlNode.SelectNodes(ApplyMappingNamespacePrefix(XML_PROPERTY),
                        _configScope.XmlNamespaceManager))
                    {
        ...
        private void ProcessSqlStatement(IStatement statement)
        {
            ...
            if (statement.Id == "DynamicJIRA") Console.Write("tt");
            ...
            if (statement.ExtendStatement.Length > 0)
            {
                ...
                XmlNode supperStatementNode = _configScope.SqlMapDocument.SelectSingleNode(
                    ApplyMappingNamespacePrefix(XML_SEARCH_STATEMENT) + "/child::*[@id='" + statement.ExtendStatement +
                    "']", _configScope.XmlNamespaceManager);
                if (supperStatementNode != null)
                    commandTextNode.InnerXml = supperStatementNode.InnerXml + commandTextNode.InnerXml;
                else
                    throw new ConfigurationException("Unable to find extend statement named '" +
                                                     statement.ExtendStatement + "' on statement '" + statement.Id +
                                                     "'.'");
            }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                else if (child.Name == "include")
                {
                    ...
                    if (includeNode == null)
                    {
                        ...
                        if (includeNode == null)
                            throw new ConfigurationException(
                                "Could not find SQL tag to include with refid '" + refid + "'");
                    }
        ...
        private void ApplyInlineParemeterMap(IStatement statement, string sqlStatement)
        {
            ...
            if (statement.ParameterMap == null)
            {
                ...
                if (sqlText.Parameters.Length > 0)
                {
                    ...
                    if (statement.ParameterClass != null) map.Class = statement.ParameterClass;
                    map.Initialize(_configScope.DataSource.DbProvider.UsePositionalParameters, _configScope);
                    if (statement.ParameterClass == null &&
                        sqlText.Parameters.Length == 1 &&
                        sqlText.Parameters[0].PropertyName == "value"
                    ) //#value# parameter with no parameterClass attribut
                        map.DataExchange =
                            _configScope.DataExchangeFactory
                                .GetDataExchangeForClass(typeof(int)); //Get the primitiveDataExchange
                    statement.ParameterMap = map;

                    int lenght = sqlText.Parameters.Length;
                    for (int index = 0; index < lenght; index++) map.AddParameterProperty(sqlText.Parameters[index]);
                }
        ...
        private void ParseGlobalProperties()
        {
            XmlNode nodeProperties =
                _configScope.NodeContext.SelectSingleNode(ApplyDataMapperNamespacePrefix(XML_PROPERTIES),
                    _configScope.XmlNamespaceManager);
            ...
            if (nodeProperties != null)
            {
                ...
                {
                    foreach (XmlNode propertyNode in nodeProperties.SelectNodes(
                        ApplyDataMapperNamespacePrefix(XML_PROPERTY), _configScope.XmlNamespaceManager))
                    {
                        ...
                        if (keyAttrib != null && valueAttrib != null)
                        {
                            ...
                            if (_logger.IsDebugEnabled)
                                _logger.Debug(string.Format("Add property \"{0}\" value \"{1}\"", keyAttrib.Value,
                                    valueAttrib.Value));
                        }
                        else
                        {
                            // Load the file defined by the attribute
                            XmlDocument propertiesConfig =
                                Resources.GetAsXmlDocument(propertyNode, _configScope.Properties);
                                ...
                                _configScope.XmlNamespaceManager))
                            {
                                ...
                                if (_logger.IsDebugEnabled)
                                    _logger.Debug(string.Format("Add property \"{0}\" value \"{1}\"",
                                        node.Attributes[PROPERTY_ELEMENT_KEY_ATTRIB].Value,
                                        node.Attributes[PROPERTY_ELEMENT_VALUE_ATTRIB].Value));
                            }
                        }
                    }
        ...
        private void BuildParameterMap()
        {
            ...
            if (_configScope.SqlMapper.ParameterMaps.Contains(id) == false)
            {
                ...
                if (parameterMap.ExtendMap.Length > 0)
                {
                    ...
                    {
                        XmlNode superNode = _configScope.SqlMapConfigDocument.SelectSingleNode(
                            ApplyMappingNamespacePrefix(XML_SEARCH_PARAMETER) + attributeExtendMap + "']",
                            _configScope.XmlNamespaceManager);
                        ...
                        {
                            throw new ConfigurationException("In mapping file '" + _configScope.SqlMapNamespace +
                                                             "' the parameterMap '" + parameterMap.Id +
        ...
        private void BuildResultMap()
        {
            ...
            if (_configScope.SqlMapper.ResultMaps.Contains(id) == false)
            {
                ...
                if (resultMap.ExtendMap != null && resultMap.ExtendMap.Length > 0)
                {
                    ...
                    {
                        XmlNode superNode = _configScope.SqlMapDocument.SelectSingleNode(
                            ApplyMappingNamespacePrefix(XML_SEARCH_RESULTMAP) + attributeExtendMap + "']",
                            _configScope.XmlNamespaceManager);
        ...
        public Stream GetStream(string schemaResourceKey)
        {
            return Assembly.GetExecutingAssembly()
                .GetManifestResourceStream("IBatisNet.DataMapper." + schemaResourceKey);
        }
        ...
        {
            return DATAMAPPER_NAMESPACE_PREFIX + ":" +
                   elementName.Replace("/", "/" + DATAMAPPER_NAMESPACE_PREFIX + ":");
        }
        ...
        {
            return PROVIDERS_NAMESPACE_PREFIX + ":" + elementName.Replace("/", "/" + PROVIDERS_NAMESPACE_PREFIX + ":");
        }
        ...
        {
            return MAPPING_NAMESPACE_PREFIX + ":" + elementName.Replace("/", "/" + MAPPING_NAMESPACE_PREFIX + ":");
        }

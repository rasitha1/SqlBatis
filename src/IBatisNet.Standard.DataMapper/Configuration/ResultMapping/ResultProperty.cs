...
/*****************************************************************************
 * $Revision: 474455 $
 * $LastChangedDate: 2006-11-13 20:30:00 +0100 (lun., 13 nov. 2006) $
...
namespace IBatisNet.DataMapper.Configuration.ResultMapping
{
    ...
    public class ResultProperty
    {
        ...
        [NonSerialized] private ISetAccessor _setAccessor;

        [NonSerialized] private string _nullValue;

        [NonSerialized] private string _propertyName = string.Empty;

        [NonSerialized] private string _columnName = string.Empty;

        [NonSerialized] private int _columnIndex = UNKNOWN_COLUMN_INDEX;

        [NonSerialized] private string _select = string.Empty;

        [NonSerialized] private string _nestedResultMapName = string.Empty;

        [NonSerialized] private IResultMap _nestedResultMap;

        [NonSerialized] private string _dbType;

        [NonSerialized] private string _clrType = string.Empty;

        [NonSerialized] private bool _isLazyLoad;

        [NonSerialized] private ITypeHandler _typeHandler;

        [NonSerialized] private string _callBackName = string.Empty;

        [NonSerialized] private bool _isComplexMemberName;

        [NonSerialized] private IPropertyStrategy _propertyStrategy;

        [NonSerialized] private ILazyFactory _lazyFactory;

        [NonSerialized] private bool _isIList;

        [NonSerialized] private bool _isGenericIList;

        [NonSerialized] private IFactory _listFactory;

        [NonSerialized] private static readonly IFactory _arrayListFactory = new ArrayListFactory();
        ...
        /// </summary>
        [XmlIgnore]
        public bool IsGenericIList => _isGenericIList;

        /// <summary>
        ///     Tell us if the member type implement Ilist interface.
        /// </summary>
        [XmlIgnore]
        public bool IsIList => _isIList;
        ...
        [XmlIgnore]
        public IFactory ListFactory => _listFactory;

        /// <summary>
        ///     The lazy loader factory
        /// </summary>
        [XmlIgnore]
        public ILazyFactory LazyFactory => _lazyFactory;
            ...
            get
            {
                if (_typeHandler == null)
                    throw new DataMapperException(
                        string.Format("Error on Result property {0}, type handler for {1} is not registered.",
                            PropertyName, MemberType.Name));
                return _typeHandler;
            }
        ...
        /// </summary>
        [XmlIgnore]
        public virtual Type MemberType => _setAccessor.MemberType;
        ...
        /// </summary>
        public bool IsComplexMemberName => _isComplexMemberName;
        ...
        public void Initialize(ConfigurationScope configScope, Type resultClass)
        {
                ...
                !typeof(IDictionary).IsAssignableFrom(resultClass))
            {
                ...
                {
                    _setAccessor =
                        configScope.DataExchangeFactory.AccessorFactory.SetAccessorFactory.CreateSetAccessor(
                            resultClass, _propertyName);
                }
                else // complex member name FavouriteLineItem.Id
                {
                    ...
                    _setAccessor =
                        configScope.DataExchangeFactory.AccessorFactory.SetAccessorFactory.CreateSetAccessor(
                            propertyInfo.ReflectedType, memberName);
                }
                ...
                if (_isGenericIList)
                {
                    ...
                    else
                    {
                        ...
                        {
                            _listFactory =
                                configScope.DataExchangeFactory.ObjectFactory.CreateFactory(MemberType,
                                    Type.EmptyTypes);
                        }
                        else
                        {
            ...
            {
                configScope.ErrorContext.MoreInfo = "Result property '" + _propertyName +
            ...
            else
            {
                configScope.ErrorContext.MoreInfo =
                    "Result property '" + _propertyName + "' set the typeHandler attribute.";
                _typeHandler = configScope.ResolveTypeHandler(resultClass, _propertyName, _clrType, _dbType, true);
            }

            if (IsLazyLoad)
                _lazyFactory =
                    new LazyFactoryBuilder(configScope.LazyFactoryType).GetLazyFactory(_setAccessor.MemberType);
        }

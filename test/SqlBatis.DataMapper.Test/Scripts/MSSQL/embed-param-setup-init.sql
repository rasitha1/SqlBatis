...
begin
    if (select count(1) from [dbo].[ManyRecords]) <= 1000000
	begin
...
if not exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[ManyRecords]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
begin

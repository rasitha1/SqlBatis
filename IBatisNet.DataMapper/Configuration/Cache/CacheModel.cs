...
/*****************************************************************************
 * $Revision: 492266 $
 * $LastChangedDate: 2007-01-03 20:36:11 +0100 (mer., 03 janv. 2007) $
...
namespace IBatisNet.DataMapper.Configuration.Cache
{
    ...
    public class CacheModel
    {
        ...
        private static readonly IDictionary _lockMap = new HybridDictionary();
        ...
        /// </summary>
        [NonSerialized] public readonly static object NULL_OBJECT = new object();

        /// <summary>
        ///     Constant to turn off periodic cache flushes
        /// </summary>
        [NonSerialized] public const long NO_FLUSH_INTERVAL = -99999;

        [NonSerialized] private readonly object _statLock = new object();

        [NonSerialized] private int _requests;

        [NonSerialized] private int _hits;

        [NonSerialized] private string _id = string.Empty;

        [NonSerialized] private ICacheController _controller;

        [NonSerialized] private FlushInterval _flushInterval;

        [NonSerialized] private long _lastFlush;

        [NonSerialized] private readonly HybridDictionary _properties = new HybridDictionary();

        [NonSerialized] private string _implementation = string.Empty;

        [NonSerialized] private bool _isReadOnly = true;

        [NonSerialized] private bool _isSerializable;
        ...
        public void Initialize()
        {
            ...
            try
            {
                if (_implementation == null)
                    throw new DataMapperException("Error instantiating cache controller for cache named '" + _id +
                                                  "'. Cause: The class for name '" + _implementation +
                                                  "' could not be found.");
            ...
            catch (Exception e)
            {
                throw new ConfigurationException(
                    "Error instantiating cache controller for cache named '" + _id + ". Cause: " + e.Message, e);
            }
            ...
            {
                throw new ConfigurationException(
                    "Error configuring controller named '" + _id + "'. Cause: " + e.Message, e);
            }
        }

    ...
    public class Resources
    {
        ...
        internal class FileAssemblyInfo
        {
            ...
            private void SplitFileAndAssemblyNames(string originalFileName)
            {
                ...
                int separatorIndex = originalFileName.IndexOf(FileAssemblySeparator);

                if (separatorIndex < 0)
                {
                    FileName = originalFileName.Trim();
                    AssemblyName = null; // IsAssemblyQualified will return false
                }
                else
                {
                    FileName = originalFileName.Substring(0, separatorIndex).Trim();
                    AssemblyName = originalFileName.Substring(separatorIndex + 1).Trim();
                }
            }
        ...
        /// </summary>
        public static string ApplicationBase { get; } = AppDomain.CurrentDomain.SetupInformation.ApplicationBase;
        ...
        public static XmlDocument GetConfigAsXmlDocument(string resourcePath)
        {
            ...
            if (!FileExists(resourcePath)) resourcePath = Path.Combine(BaseDirectory, resourcePath);
        ...
        public static XmlDocument GetAsXmlDocument(XmlNode node, NameValueCollection properties)
        {
            ...
            else if (node.Attributes["embedded"] != null)
            {
                ...
                xmlDocument = GetEmbeddedResourceAsXmlDocument(embedded);
            }
        ...
        public static XmlDocument GetResourceAsXmlDocument(string resource)
        {
            ...
            try
            {
                config.Load(Path.Combine(ApplicationBase, resource));
            }
            catch (Exception e)
            {
                throw new ConfigurationException(
        ...
        public static XmlDocument GetEmbeddedResourceAsXmlDocument(string resource)
        {
            ...
            if (fileInfo.IsAssemblyQualified)
            {
                ...
                // JIRA - IBATISNET-103 
                if (stream == null) stream = assembly.GetManifestResourceStream(fileInfo.FileName);
                if (stream != null)
                    try
                    {
                        config.Load(stream);
        ...
        public static FileInfo GetFileInfo(string resourcePath)
        {
            ...
            if (!FileExists(resourcePath)) resourcePath = Path.Combine(ApplicationBase, resourcePath);
